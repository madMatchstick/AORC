...
#include <stdlib.h>
#include <stdbool.h>
#include "../include/read_aorc_forcing.h"
#include "../include/bmi.h"
#include "../include/bmi_aorc.h"

#define INPUT_VAR_NAME_COUNT 7 // All the forcings
#define OUTPUT_VAR_NAME_COUNT 7 // All the forcings
...
Initialize (Bmi *self, const char *cfg_file)
{
    ...
    aorc->forcing_data_precip_kg_per_m2 = malloc(sizeof(double) * (aorc->num_timesteps + 1));
    aorc->forcing_data_surface_pressure_Pa = malloc(sizeof(double) * (aorc->num_timesteps + 1));
    aorc->forcing_data_time = malloc(sizeof(long) * (aorc->num_timesteps + 1));
    aorc->forcing_data_incoming_shortwave_W_per_m2 = malloc(sizeof(double) * (aorc->num_timesteps + 1));
    aorc->forcing_data_incoming_longwave_W_per_m2 = malloc(sizeof(double) * (aorc->num_timesteps + 1));
    aorc->forcing_data_specific_humidity_2m_kg_per_kg = malloc(sizeof(double) * (aorc->num_timesteps + 1));
    aorc->forcing_data_air_temperature_2m_K = malloc(sizeof(double) * (aorc->num_timesteps + 1));
    aorc->forcing_data_u_wind_speed_10m_m_per_s = malloc(sizeof(double) * (aorc->num_timesteps + 1));
    aorc->forcing_data_v_wind_speed_10m_m_per_s = malloc(sizeof(double) * (aorc->num_timesteps + 1));
    ...
    for (int i = 0; i < aorc->num_timesteps; i++) {
        ...
        aorc->forcing_data_precip_kg_per_m2[i] = aorc->precip_kg_per_m2 * ((float)aorc->time_step_size);
        if (aorc->verbose >4)
            printf("precip %f \n", aorc->forcing_data_precip_kg_per_m2[i]);
        aorc->forcing_data_surface_pressure_Pa[i] = aorc->surface_pressure_Pa;
        if (aorc->verbose >4)
            printf("surface pressure %f \n", aorc->forcing_data_surface_pressure_Pa[i]);
        aorc->forcing_data_incoming_longwave_W_per_m2[i] = aorc->incoming_longwave_W_per_m2;
        if (aorc->bmi.verbose >4)
            printf("longwave %f \n", et->forcing_data_incoming_longwave_W_per_m2[i]);
        aorc->forcing_data_incoming_shortwave_W_per_m2[i] = aorc->incoming_shortwave_W_per_m2;
        if (aorc->bmi.verbose >4)
            printf("shortwave %f \n", aorc->forcing_data_incoming_shortwave_W_per_m2[i]);
        aorc->forcing_data_specific_humidity_2m_kg_per_kg[i] = aorc->specific_humidity_2m_kg_per_kg;
        if (aorc->verbose >4)
            printf("humidity %f \n", aorc->forcing_data_specific_humidity_2m_kg_per_kg[i]);
        aorc->forcing_data_air_temperature_2m_K[i] = aorc->air_temperature_2m_K;
        if (aorc->verbose >4)
            printf("air temperature %f \n", aorc->forcing_data_air_temperature_2m_K[i]);
        aorc->forcing_data_u_wind_speed_10m_m_per_s[i] = aorc->u_wind_speed_10m_m_per_s;
        if (aorc->verbose >4)
            printf("u wind speed %f \n", aorc->forcing_data_u_wind_speed_10m_m_per_s[i]);
        aorc->forcing_data_v_wind_speed_10m_m_per_s[i] = aorc->v_wind_speed_10m_m_per_s;
        if (aorc->verbose >4)
            printf("v wind speed %f \n", aorc->forcing_data_v_wind_speed_10m_m_per_s[i]);


        aorc->forcing_data_time[i] = aorc->time;
        if (i == 0)
            aorc->current_time = aorc->time;
    }
...
Update (Bmi *self)
{
    aorc_forcing *aorc = (aorc_forcing *) self->data;
    ...
    aorc->current_time_step += aorc->time_step_size; // Seconds since start of run
    aorc->current_step +=1;                            // time steps since start of run
    aorc->current_time += aorc->time_step_size;   // Seconds since 1970
...
//---------------------------------------------------------------------------------------------------------------------
int read_file_line_counts_et(const char* file_name, int* line_count, int* max_line_length)
{
...
int read_init_config_et(et_model* model, const char* config_file)//,
{
    ...
    int count_result = read_file_line_counts_et(config_file, &config_line_count, &max_config_line_length);
    if (count_result == -1) {
        return BMI_FAILURE;
    }
