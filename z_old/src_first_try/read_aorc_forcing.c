#ifndef READ_FORCING_C
#define READ_FORCING_C
...
// ######################    RUN    ########    RUN    ########    RUN    ########    RUN    #################################
// ######################    RUN    ########    RUN    ########    RUN    ########    RUN    #################################
extern int run_aorc(aorc_forcing* aorc)
{

  // populate the evapotranspiration forcing data structure:
  //---------------------------------------------------------------------------------------------------------------
  aorc->air_temperature_C = aorc->forcing_data_air_temperature_2m_K[aorc->current_step] - TK;//convert to C
  aorc->relative_humidity_percent     = (double)-99.9; // this negative number means use specific humidity
  aorc->specific_humidity_2m_kg_per_kg = aorc->forcing_data_precip_kg_per_m2[aorc->current_step];
  aorc->air_pressure_Pa    = aorc->forcing_data_surface_pressure_Pa[aorc->current_step];
  aorc->wind_speed_m_per_s = hypot(aorc->forcing_data_u_wind_speed_10m_m_per_s[aorc->current_step],
                                         aorc->forcing_data_v_wind_speed_10m_m_per_s[aorc->current_step]);                 

  
/************************************************************************************************************************************/
/************************************************************************************************************************************/
/************************************************************************************************************************************/
  aorc->incoming_longwave_W_per_m2     =  aorc->forcing_data_incoming_longwave_W_per_m2[aorc->current_step];
  aorc->incoming_shortwave_W_per_m2    =  aorc->forcing_data_incoming_shortwave_W_per_m2[aorc->current_step];
  aorc->surface_pressure_Pa            =  aorc->forcing_data_surface_pressure_Pa[aorc->current_step];
  aorc->specific_humidity_2m_kg_per_kg =  aorc->forcing_data_specific_humidity_2m_kg_per_kg[aorc->current_step];

  aorc->air_temperature_2m_K           =  aorc->forcing_data_air_temperature_2m_K[aorc->current_step];
  aorc->u_wind_speed_10m_m_per_s       =  aorc->forcing_data_u_wind_speed_10m_m_per_s[aorc->current_step];
  aorc->v_wind_speed_10m_m_per_s       =  aorc->forcing_data_v_wind_speed_10m_m_per_s[aorc->bmi.current_step];
